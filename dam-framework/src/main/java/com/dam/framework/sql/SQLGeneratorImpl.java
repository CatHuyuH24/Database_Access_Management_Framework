...
public final class SQLGeneratorImpl implements SQLGenerator {
    ...
    public String generateInsert(EntityMetadata metadata) {
        ...
        // Get columns excluding auto-generated ID
        List<ColumnMetadata> columns = metadata.getColumns().stream()
                .filter(col -> {
                    // Include column if:
                    // 1. It's not the ID column, OR
                    // 2. It's the ID but not auto-generated
                    if (!col.equals(idColumn)) {
                        return true;
                    }

                    // Check if ID has @GeneratedValue
                    // TODO: Add isAutoGenerated() method to ColumnMetadata

                    // For now, assume all IDs are auto-generated and ALWAYS EXCLUDE ID COLUMNS
                    return false;
                })
