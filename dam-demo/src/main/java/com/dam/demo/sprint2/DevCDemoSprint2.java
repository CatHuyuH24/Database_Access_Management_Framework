...
public class DevCDemoSprint2 {
  ...
  private static void demoTypeMapper() {
    ...
    System.out.println("   String.class       → " + TypeMapper.getJdbcType(String.class) + " (VARCHAR)");
    System.out.println("   Integer.class      → " + TypeMapper.getJdbcType(Integer.class) + " (INTEGER)");
    System.out.println("   Boolean.class      → " + TypeMapper.getJdbcType(Boolean.class) + " (BOOLEAN)");
    System.out.println("   LocalDate.class    → " + TypeMapper.getJdbcType(LocalDate.class) + " (DATE)");
    System.out.println("   LocalDateTime.class→ " + TypeMapper.getJdbcType(LocalDateTime.class) + " (TIMESTAMP)");
    System.out.println("   BigDecimal.class   → " + TypeMapper.getJdbcType(BigDecimal.class) + " (DECIMAL)");

    // JDBC → Java Type Mapping
    System.out.println("\n✅ JDBC → Java Type Mapping:");
    System.out.println("   VARCHAR (12)       → " + TypeMapper.getJavaType(Types.VARCHAR).getSimpleName());
    System.out.println("   INTEGER (4)        → " + TypeMapper.getJavaType(Types.INTEGER).getSimpleName());
    System.out.println("   BOOLEAN (16)       → " + TypeMapper.getJavaType(Types.BOOLEAN).getSimpleName());
    System.out.println("   DATE (91)          → " + TypeMapper.getJavaType(Types.DATE).getSimpleName());
    System.out.println("   TIMESTAMP (93)     → " + TypeMapper.getJavaType(Types.TIMESTAMP).getSimpleName());
    System.out.println("   DECIMAL (3)        → " + TypeMapper.getJavaType(Types.DECIMAL).getSimpleName());

    // Type Checkers
    System.out.println("\n✅ Type Checkers:");
    System.out.println("   isPrimitiveOrWrapper(int.class)     → " + TypeMapper.isPrimitiveOrWrapper(int.class));
    System.out.println("   isNumericType(BigDecimal.class)     → " + TypeMapper.isNumericType(BigDecimal.class));
    System.out.println("   isStringType(String.class)          → " + TypeMapper.isStringType(String.class));
    System.out.println("   isDateTimeType(LocalDate.class)     → " + TypeMapper.isDateTimeType(LocalDate.class));
  }
  ...
  private static void demoEnhancedMySQLDialect() {
    System.out.println("🔧 DEMO 2: Enhanced MySQLDialect - Type Mapping Integration");
    System.out.println("-".repeat(60));

    MySQLDialect dialect = (MySQLDialect) DialectFactory.createDialect(DialectType.MYSQL);

    // Java → JDBC Type
    System.out.println("✅ Java → JDBC Type (via MySQLDialect):");
    System.out.println("   String.class       → JDBC Type " + dialect.getJdbcTypeForJavaType(String.class));
    System.out.println("   Integer.class      → JDBC Type " + dialect.getJdbcTypeForJavaType(Integer.class));
    System.out.println("   LocalDate.class    → JDBC Type " + dialect.getJdbcTypeForJavaType(LocalDate.class));

    // JDBC → Java Type
    System.out.println("\n✅ JDBC → Java Type (via MySQLDialect):");
    System.out.println("   JDBC VARCHAR       → " + dialect.getJavaTypeForJdbcType(Types.VARCHAR).getSimpleName());
    System.out.println("   JDBC INTEGER       → " + dialect.getJavaTypeForJdbcType(Types.INTEGER).getSimpleName());
    System.out.println("   JDBC DATE          → " + dialect.getJavaTypeForJdbcType(Types.DATE).getSimpleName());

    // Java → MySQL Type (one-step conversion)
    System.out.println("\n✅ Java → MySQL Type (direct conversion):");
    System.out.println("   String.class (100) → " + dialect.getMySQLTypeForJavaType(String.class, 100));
    System.out.println("   Integer.class      → " + dialect.getMySQLTypeForJavaType(Integer.class, 0));
    System.out.println("   Boolean.class      → " + dialect.getMySQLTypeForJavaType(Boolean.class, 0));
    System.out.println("   LocalDate.class    → " + dialect.getMySQLTypeForJavaType(LocalDate.class, 0));
    System.out.println("   BigDecimal.class   → " + dialect.getMySQLTypeForJavaType(BigDecimal.class, 10));
  }
  ...
  private static void demoConnectionPoolMonitoring() {
    ...
    BasicConnectionManager connectionManager = (BasicConnectionManager) new BasicConnectionManager.Builder()
  ...
  private static void demoFullIntegration() {
    ...
    try {
      ...
      String insertSQL = "INSERT INTO `employees` (`name`, `salary`, `hire_date`, `last_login`, `is_active`) " +
          "VALUES (?, ?, ?, ?, ?)";
      ...
      pstmt.close();
      System.out.println("✅ 2 employees inserted using type-safe PreparedStatement");

      // Query with TypeMapper for result extraction
      ResultSet rs = conn.createStatement().executeQuery("SELECT * FROM `employees` ORDER BY `salary` DESC");
      ...
      while (rs.next()) {
        ...
        BigDecimal salary = TypeMapper.getResultSetValue(rs, "salary", BigDecimal.class);
        LocalDate hireDate = TypeMapper.getResultSetValue(rs, "hire_date", LocalDate.class);
        Boolean active = TypeMapper.getResultSetValue(rs, "is_active", Boolean.class);
      ...
      System.out.println("   Total Connections Created:  " + stats.connectionsCreated);
      System.out.println("   Total Connections Acquired: " + stats.connectionsAcquired);
      System.out.println("   Total Connections Released: " + stats.connectionsReleased);
      System.out
          .println("   Pool Health:                " + (connectionManager.isHealthy() ? "✅ HEALTHY" : "❌ UNHEALTHY"));
